...
readonly app_dir="${CATLAS_APP_DIR:-$project_dir/Catlas/v4}"
...
readonly catlas_port="${CATLAS_PORT:-4510}"
...
if [[ ! -d "$app_dir" ]]; then
  echo "CRC Atlas v4 app directory does not exist: $app_dir" >&2
  exit 66
fi

if [[ ! -r "$app_dir/app.R" ]]; then
  echo "CRC Atlas v4 app entrypoint is not readable: $app_dir/app.R" >&2
  exit 66
fi
...
echo "Starting CRC Atlas v4 with R: $r_bin" >&2
