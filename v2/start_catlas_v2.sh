...
readonly default_project_dir="/home/minwook/Projects/BRL/SNHG16/Shiny_CRC_atlas"
readonly project_dir="${CATLAS_PROJECT_DIR:-$default_project_dir}"
readonly app_dir="${CATLAS_APP_DIR:-$project_dir/v2}"
readonly data_path="${CATLAS_DATA_PATH:-$project_dir/crc_shiny_app_seurat.rds}"
readonly tmp_root="${CATLAS_TMP_ROOT:-$project_dir/tmp_for_catlas}"
readonly catlas_host="${CATLAS_HOST:-127.0.0.1}"
readonly catlas_port="${CATLAS_PORT:-3838}"
...
if [[ ! -d "$app_dir" ]]; then
  echo "CRC Atlas v2 app directory does not exist: $app_dir" >&2
  exit 66
fi

if [[ ! -r "$app_dir/app.R" ]]; then
  echo "CRC Atlas v2 app entrypoint is not readable: $app_dir/app.R" >&2
  exit 66
fi
...
if [[ -n "${CATLAS_R_BIN:-}" ]]; then
  r_bin="$CATLAS_R_BIN"
elif [[ -x /home/minwook/anaconda3/envs/crc_shiny/bin/R ]]; then
  r_bin="/home/minwook/anaconda3/envs/crc_shiny/bin/R"
else
...
echo "Starting CRC Atlas v2 with R: $r_bin" >&2
